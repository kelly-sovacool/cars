...
```{r}
numericInput("price_EV", "EV purchase price", min=0, step=500, value=27500)
...
numericInput("energy_price_per_kWh", "Electricity price ($/kWh",min=0,max=1, value=0.25, step=0.01)
```
...
#| panel: fill
tableOutput('cars')
plotOutput('car_costs_plot')
#tableOutput('car_costs_dat')
```
...
output$car_costs_plot <- renderPlot({
    cars_costs_dat() %>% 
        ggplot(aes(miles, cost_at_mileage, color = car, group = car)) +
        geom_line() +
...
#output$car_costs_dat <- renderTable(cars_costs_dat())
```
